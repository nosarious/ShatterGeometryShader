...
uniform float shatterTime = 0; //Amount of shatter
...
void main(void)
{
	...
	N.z /= 1.0 + distort;
		...
		//N.y /= 1.0 + (shatterTime* sin( phase + 0.01 * p0.y ));
		N.y -= shatterTime/20;
		//N.x += shatterTime*distort;
	/*
	float distort2 = distortAmount * cos( phase/2 + 0.01 * p0.x );
	...
	*/
	...
	for (int i = 0; i < 3; i++)
		{
		gl_Position = gl_PositionIn[i] + vec4(shatterTime*N, 0.0);
			//if (gl_Position.y<-250) gl_Position.y = gl_Position.y+100;
		if (gl_Position.y<-250) gl_Position.y = -220;
		EmitVertex();
		}
	/*
